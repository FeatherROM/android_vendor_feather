...
func init() {
	android.RegisterModuleType("feather_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = featherExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = featherExpandVariables(ctx, dep_file)
	...
	cmd := featherExpandVariables(ctx, String(g.properties.Cmd))
