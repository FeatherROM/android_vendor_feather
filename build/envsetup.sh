...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Feather_*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.feather.device | grep -q "$FEATHER_DEVICE"); then
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.feather.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/FeatherROM/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.feather.device | grep -q "$FEATHER_DEVICE");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.feather.device | grep -q "$FEATHER_DEVICE") || [ "$FORCE_PUSH" = "true" ];
    then
